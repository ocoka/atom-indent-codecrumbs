...
export default class IndentCodeCrumbs {
  constructor(serializedState) {
    ...
    this.view = new IndentCodeCrumbsView({
      ...
      fullWidth: atom.config.get('atom-indent-codecrumbs.fullWidth'),
      crumbsClick: this.goToPosition.bind(this)
    });
    this.subscriptions = new CompositeDisposable(
      atom.workspace.observeActiveTextEditor(this.onTextEditor.bind(this)),
      atom.config.onDidChange('atom-indent-codecrumbs', ({oldValue, newValue}) => {
        this.view.update({
          color: newValue.colorCrumbs,
          fullWidth: newValue.fullWidth
        })
      })
    );
  }
